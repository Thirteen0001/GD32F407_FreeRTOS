...
#define CLK(bit) 	gpio_bit_write(SPI2_CLK_PORT, SPI2_CLK_PIN, ((bit) == 0) ? RESET : SET)
#define MOSI(bit) 	gpio_bit_write(SPI2_MOSI_PORT,SPI2_MOSI_PIN, ((bit) == 0) ? RESET : SET)

#if SPI2_MISO_ENABLE
#define MISO_STATE()  gpio_input_bit_get(SPI2_MISO_PORT, SPI2_MISO_PIN)
#endif

void lib_spi2_init() {
    /*********** GPIO config ***************/
    // clk
    rcu_periph_clock_enable(SPI2_CLK_PORT_RCU);
    gpio_mode_set(SPI2_CLK_PORT, GPIO_MODE_OUTPUT, GPIO_PUPD_PULLUP, SPI2_CLK_PIN);
    gpio_output_options_set(SPI2_CLK_PORT, GPIO_OTYPE_PP, GPIO_OSPEED_50MHZ, SPI2_CLK_PIN);

#if SPI2_MOSI_ENABLE
    // mosi
    rcu_periph_clock_enable(SPI2_MOSI_PORT_RCU);
    gpio_mode_set(SPI2_MOSI_PORT, GPIO_MODE_OUTPUT, GPIO_PUPD_PULLUP, SPI2_MOSI_PIN);
    gpio_output_options_set(SPI2_MOSI_PORT, GPIO_OTYPE_PP, GPIO_OSPEED_50MHZ, SPI2_MOSI_PIN);
#endif
#if SPI2_MISO_ENABLE
    // miso
    rcu_periph_clock_enable(SPI2_MISO_PORT_RCU);
    gpio_mode_set(SPI2_MISO_PORT, GPIO_MODE_INPUT, GPIO_PUPD_PULLUP, SPI2_MISO_PIN);
    gpio_output_options_set(SPI2_MISO_PORT, GPIO_OTYPE_PP, GPIO_OSPEED_50MHZ, SPI2_MISO_PIN);
#endif
}


#if SPI2_MOSI_ENABLE
void lib_spi2_write(uint8_t dat) {
    uint8_t i;
    for(i=0; i<8; i++)
    {
        CLK(0);
...
#define DMAx_IRQ	DMA0_Channel5_IRQn
...
static void dma_tx_config() {
	...
	nvic_irq_enable(DMAx_IRQ, SPI2_DMA_TX_NVIC);
...
void lib_spi2_init() {
    ...
    // clk
    rcu_periph_clock_enable(SPI2_CLK_PORT_RCU);
    gpio_mode_set(SPI2_CLK_PORT, GPIO_MODE_AF, GPIO_PUPD_PULLUP, SPI2_CLK_PIN);
    gpio_output_options_set(SPI2_CLK_PORT, GPIO_OTYPE_PP, GPIO_OSPEED_50MHZ, SPI2_CLK_PIN);
    gpio_af_set(SPI2_CLK_PORT, SPI2_CLK_AF, SPI2_CLK_PIN);

#if SPI2_MOSI_ENABLE
    // mosi
    rcu_periph_clock_enable(SPI2_MOSI_PORT_RCU);
    gpio_mode_set(SPI2_MOSI_PORT, GPIO_MODE_AF, GPIO_PUPD_PULLUP, SPI2_MOSI_PIN);
    gpio_output_options_set(SPI2_MOSI_PORT, GPIO_OTYPE_PP, GPIO_OSPEED_50MHZ, SPI2_MOSI_PIN);
    gpio_af_set(SPI2_MOSI_PORT, SPI2_MOSI_AF, SPI2_MOSI_PIN);
#endif
#if SPI2_MISO_ENABLE
    // miso
    rcu_periph_clock_enable(SPI2_MISO_PORT_RCU);
    ...
    gpio_output_options_set(SPI2_MISO_PORT, GPIO_OTYPE_PP, GPIO_OSPEED_50MHZ, SPI2_MISO_PIN);
    gpio_af_set(SPI2_MISO_PORT, SPI2_MISO_AF, SPI2_MISO_PIN);
#endif
    /*************** spi config **************/
    // 时钟
    ...
    spi_init_struct.clock_polarity_phase = SPI2_PARAM_CPOL_CPHA;
    spi_init_struct.prescale             = SPI2_PARAM_PSC;
    spi_init_struct.endian               = SPI2_PARAM_ENDIAN;
