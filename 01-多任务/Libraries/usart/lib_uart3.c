...
static uint8_t uart3_recv_buff[UART3_RECV_LEN];   // 接收缓冲区
static uint32_t uart3_recv_len = 0;
...
static void DMA_tx_config() {
    ...
    uint32_t dma_tx = UART3_DMA_TX;
    uint32_t dma_tx_chn = UART3_DMA_TX_CHN;
    uint32_t dma_tx_direction = DMA_MEMORY_TO_PERIPH;
    uint32_t dma_tx_dst_addr = (uint32_t)(&USART_DATA(UART3));
    uint32_t dam_tx_subperi = UART3_DMA_TX_SUBPERI;
    /********************* DMA TX ***************************/
    // 配置时钟
...
static void DMA_rx_config() {
    uint32_t dma_rx_rcu = UART3_DMA_RX_RCU;
    uint32_t dma_rx = UART3_DMA_RX;
    uint32_t dma_rx_chn = UART3_DMA_RX_CHN;
    uint32_t dma_rx_direction = DMA_PERIPH_TO_MEMORY;
    uint32_t dma_rx_src_addr = (uint32_t)(&USART_DATA(UART3));
    uint32_t dma_rx_dst_addr = (uint32_t)uart3_recv_buff;
    uint32_t dam_rx_subperi = UART3_DMA_RX_SUBPERI;
    /******************** DMA RX *******************/
    // 配置时钟
    ...
    dsdps.number = UART3_RECV_LEN; //传输的数据个数
...
void lib_uart3_init() {
    uint32_t usartx = UART3;
    uint32_t usartx_rcu = RCU_UART3;
    uint32_t usartx_irqn = UART3_IRQn;

    uint32_t tx_gpio_rcu = UART3_TX_RCU;
    uint32_t tx_gpio_port = UART3_TX_PORT;
    uint32_t tx_gpio_pin = UART3_TX_PIN;
    uint32_t tx_gpio_af = UART3_TX_AF;

    uint32_t rx_gpio_rcu = UART3_RX_RCU;
    uint32_t rx_gpio_port = UART3_RX_PORT;
    uint32_t rx_gpio_pin = UART3_RX_PIN;
    uint32_t rx_gpio_af = UART3_RX_AF;

    uint32_t baudrate = UART3_PARAM_BAUDRATE;
    uint32_t parity = 	UART3_PARAM_PARITY;
    uint32_t word_len = UART3_PARAM_WORD_LEN;
    uint32_t stop_bit = UART3_PARAM_STOP_BIT;
    uint32_t data_first = UART3_PARAM_DATA_FIRST;
    ////////// gpio config ///////////////
    // 配置时钟
...
#if UART3_ENABLE_SEND
    // 发送功能配置
    usart_transmit_config(usartx, USART_TRANSMIT_ENABLE);
#endif

#if UART3_ENABLE_RECV
    // 接收功能配置
    usart_receive_config(usartx, USART_RECEIVE_ENABLE);
    // 接收中断配置
    nvic_irq_enable(usartx_irqn, UART3_RECV_NVIC_IRQ);
    // usart int rbne
...
#if UART3_ENABLE_DMA_SEND
static void Uart3_dma_send(uint8_t* data, uint32_t len) {
	if(data == NULL || len == 0) return;
	
    uint32_t dma_tx_rcu = UART3_DMA_TX_RCU;
    uint32_t dma_tx = UART3_DMA_TX;
    uint32_t dma_tx_chn = UART3_DMA_TX_CHN;
    uint32_t dma_tx_direction = DMA_MEMORY_TO_PERIPH;
    uint32_t dma_tx_dst_addr = (uint32_t)(&USART_DATA(UART3));
...
void lib_uart3_send_string(char *data) {
    while(data && *data) {
        lib_uart3_send_data((uint8_t)(*data));
        data++;
    }
}
...
void UART3_IRQHandler(void) {
    ...
    if (usart_interrupt_flag_get(UART3, USART_INT_FLAG_IDLE) == SET) {
        ...
        uart3_recv_len = UART3_RECV_LEN - dma_transfer_number_get(dma_rx, dma_rx_chn);
        if(uart3_recv_len != 0 && uart3_recv_len < UART3_RECV_LEN)
        {
#if UART3_RECV_CALLBACK
			// 接收停止时，缓冲区中的数据，可以在此处处理缓存数据。TODO:
            lib_uart3_on_recv(uart3_recv_buff, uart3_recv_len);
#endif
            uart3_recv_buff[uart3_recv_len] = '\0';
            
            dma_flag_clear(dma_rx, dma_rx_chn, DMA_FLAG_FTF);
            dma_channel_enable(dma_rx, dma_rx_chn);
        } else {
            memset(uart3_recv_buff, 0, UART3_RECV_LEN);
        }
    }
#else
    if ((usart_interrupt_flag_get(UART3, USART_INT_FLAG_RBNE)) == SET) {
        uint16_t value = usart_data_receive(UART3);
        uart3_recv_buff[uart3_recv_len] = value;
        uart3_recv_len++;
...
int fputc(int ch, FILE *f) {
    lib_uart3_send_data((uint8_t)ch);
    return ch;
}
